...
func Load() *Config {
	return &Config{
		...
		Database: DatabaseConfig{
			Host:            getEnv("SINGLE_AUTH_DB_HOST", "single-auth-postgres-db.leetcoders.uz"),
			Port:            getEnvInt("SINGLE_AUTH_DB_PORT", 5500),
			User:            getEnv("SINGLE_AUTH_DB_USER", "single_auth"),
			Password:        getEnv("SINGLE_AUTH_DB_PASSWORD", "single_auth_secret_password"),
			Database:        getEnv("SINGLE_AUTH_DB_NAME", "single_auth_service_db"),
			SSLMode:         getEnv("SINGLE_AUTH_DB_SSL_MODE", "disable"),
			MaxOpenConns:    getEnvInt("DB_MAX_OPEN_CONNS", 25),
			MaxIdleConns:    getEnvInt("DB_MAX_IDLE_CONNS", 5),
			ConnMaxLifetime: getEnvDuration("DB_CONN_MAX_LIFETIME", 5*time.Minute),
		},
		Redis: RedisConfig{
			Host:         getEnv("SINGLE_AUTH_REDIS_HOST", "single-auth-redis-db.leetcoders.uz"),
			Port:         getEnvInt("SINGLE_AUTH_REDIS_PORT", 6600),
			Password:     getEnv("SINGLE_AUTH_REDIS_PASSWORD", "single-auth-redis-pwd"),
		...
		JWT: JWTConfig{
			...
			KeyValidityPeriod:   getEnvDuration("JWT_KEY_VALIDITY_PERIOD", 168*time.Hour),
		},
		Auth: AuthConfig{
			// Argon2id recommended parameters (OWASP)
			Argon2Memory:      getEnvUint32("ARGON2_MEMORY", 65536), // 64 MB
